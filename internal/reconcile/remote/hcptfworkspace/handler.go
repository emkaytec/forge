...
// hcp-tf-workspace kind. MK-9 ships a stub seam; MK-14 replaces it
...
// Handler implements the hcp-tf-workspace remote handler contract.
type Handler struct{}
...
func (h *Handler) DescribeChange(_ context.Context, m *schema.Manifest, source string) (reconcile.ResourceChange, error) {
	return reconcile.ResourceChange{
		...
		Note:     "hcp-tf-workspace remote handler is a stub; real reconciliation lands with the anvil carve-out",
	}, nil
}
...
func (h *Handler) Apply(_ context.Context, _ reconcile.ResourceChange, _ reconcile.ApplyOptions) error {
	return fmt.Errorf("hcp-tf-workspace: %w", reconcile.ErrNotImplemented)
}
