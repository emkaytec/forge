...
// github-repo kind. MK-9 ships a stub seam; MK-14 replaces it with
...
// Handler implements the github-repo remote handler contract.
type Handler struct{}
...
func (h *Handler) DescribeChange(_ context.Context, m *schema.Manifest, source string) (reconcile.ResourceChange, error) {
	return reconcile.ResourceChange{
		...
		Note:     "github-repo remote handler is a stub; real reconciliation lands with the anvil carve-out",
	}, nil
}
...
func (h *Handler) Apply(_ context.Context, _ reconcile.ResourceChange, _ reconcile.ApplyOptions) error {
	return fmt.Errorf("github-repo: %w", reconcile.ErrNotImplemented)
}
