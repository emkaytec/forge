...
The cobra-command shell (`forge reconcile local|remote`) is added in [MK-10](https://linear.app/wiscotrashpanda/issue/MK-10/implement-forge-reconcile-local-and-remote-commands); this package exposes the `BuildPlan`, `Executor`, `RenderPlan`, and `RenderApplyResult` entry points those commands will wire up.
